...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
    try {
        ...
        if (in_array($action, ['create_category', 'rename_category'], true)) {
            if ($name === '' || mb_strlen($name) > 80) {
                throw new InvalidArgumentException(SiteTranslations::text(
                    'O nome da categoria deve ter entre 1 e 80 caracteres.',
                    'The category name must contain between 1 and 80 characters.'
                ));
            }
        }
        ...
        if ($action === 'create_category') {
            ...
            $message = SiteTranslations::text('Categoria criada e adicionada ao menu.', 'Category created and added to the menu.');
            ...
            $message = SiteTranslations::text('Nome da categoria atualizado.', 'Category name updated.');
            ...
            $message = SiteTranslations::format(
                'Categoria apagada com {lists} e {items}.',
                'Category deleted with {lists} and {items}.',
                [
        ...
        $error = $exception->getCode() === '23000'
            ? SiteTranslations::text('Já existe uma categoria com esse nome.', 'A category with that name already exists.')
            : SiteTranslations::text('Não foi possível guardar a categoria.', 'Could not save the category.');
        ...
        $error = match (true) {
            ...
            $exception instanceof InvalidArgumentException
                && $exception->getMessage() === 'Categoria não encontrada.' => SiteTranslations::text(
            ...
            $exception instanceof RuntimeException
                && str_contains($exception->getMessage(), '022_verification_categories.sql') => SiteTranslations::text(
                    ...
                    'The categories table does not exist yet. Import migration 022_verification_categories.sql.'
                ),
...
<head>
    ...
    <title><?= categoryEscape(SiteTranslations::text('Categorias de listas', 'List categories')) ?> — <?= categoryEscape(PortalBrand::name()) ?></title>
    ...
    <script>
        window.VERIFICATION_CATEGORIES = <?= json_encode([
            ...
            'contentMessage' => SiteTranslations::text(
                'A categoria “{name}” contém {lists} e {items}. Todo esse conteúdo será apagado. Quer continuar?',
                'The “{name}” category contains {lists} and {items}. All of this content will be deleted. Do you want to continue?'
            ),
            'emptyMessage' => SiteTranslations::text(
                'Quer apagar a categoria “{name}”?',
...
<main class="lists-shell categories-shell">
    ...
    <?php if (!$storageAvailable): ?>
        <div class="notice error" role="alert"><?= categoryEscape(SiteTranslations::text(
            'Importe migrations/022_verification_categories.sql para ativar a gestão de categorias.',
            'Import migrations/022_verification_categories.sql to enable category management.'
        )) ?></div>
    <?php endif; ?>
    ...
    <details class="list-create-panel" <?= $categories === [] ? 'open' : '' ?>>
        <summary><?= categoryEscape(SiteTranslations::text('Criar nova categoria', 'Create new category')) ?></summary>
        <form method="post" class="create-list category-create-form">
            ...
            <label><span><?= categoryEscape(SiteTranslations::text('Nome da categoria', 'Category name')) ?></span><input name="name" maxlength="80" required placeholder="<?= categoryEscape(SiteTranslations::text('Ex.: Escadas', 'E.g. Stairs')) ?>" <?= $storageAvailable ? '' : 'disabled' ?>></label>
            <button type="submit" <?= $storageAvailable ? '' : 'disabled' ?>><?= categoryEscape(SiteTranslations::text('Criar categoria', 'Create category')) ?></button>
        </form>
    </details>

    <section class="category-list" aria-label="<?= categoryEscape(SiteTranslations::text('Categorias existentes', 'Existing categories')) ?>">
        <div class="category-list-heading" aria-hidden="true">
            <span><?= categoryEscape(SiteTranslations::text('Categoria', 'Category')) ?></span>
        ...
        <?php if ($categories === []): ?><p class="empty"><?= categoryEscape(SiteTranslations::text('Ainda não existem categorias.', 'There are no categories yet.')) ?></p><?php endif; ?>
        <?php foreach ($categories as $category): ?>
            <article class="category-row">
                <form method="post" class="category-rename-form">
                    ...
                    <label><span class="mobile-only"><?= categoryEscape(SiteTranslations::text('Nome da categoria', 'Category name')) ?></span><input name="name" maxlength="80" required value="<?= categoryEscape((string) $category['display_name']) ?>" aria-label="<?= categoryEscape(SiteTranslations::text('Nome da categoria', 'Category name')) ?>" <?= $storageAvailable ? '' : 'disabled' ?>></label>
