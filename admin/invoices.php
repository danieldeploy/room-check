...
function invoiceStatus(string $state, ?string $label = null): void {
    $tone = match ($state) {
        'ready', 'completed', 'ok', 'enabled', 'validated', 'drive_verified', 'alert_sent', 'alert_resolved' => 'success',
        'failed', 'drive_failed', 'alert_failed' => 'error',
        'needs_auth', 'review', 'retry', 'drive_retry', 'waiting_auth', 'not_configured', 'alert_pending', 'alert_retry' => 'warning',
        default => 'neutral',
    };
...
<header class="page-header"><h1 class="page-title"><?= it('title') ?></h1><p><?= it('scope') ?></p></header>
...
<section class="card"><h2><?= it('accounts') ?></h2><div class="invoice-table-wrap"><table role="table"><thead role="rowgroup"><tr role="row"><th scope="col" role="columnheader"><?= it('platform') ?></th><th scope="col" role="columnheader"><?= it('account') ?></th><th scope="col" role="columnheader"><?= it('property') ?></th><th scope="col" role="columnheader"><?= it('state') ?></th><th scope="col" role="columnheader"><?= it('schedule') ?></th></tr></thead><tbody role="rowgroup">
<?php foreach ($accounts as $a): ?><tr role="row"><td role="cell" data-label="<?= it('platform') ?>"><?= ie(InvoiceAccounts::PORTALS[$a['portal']]) ?></td><td role="cell" data-label="<?= it('account') ?>"><a href="?<?= ie(http_build_query(['account' => $a['id'], 'period' => $period])) ?>"><?= ie($a['label']) ?></a></td><td role="cell" data-label="<?= it('property') ?>"><?= ie(implode(', ', $properties[$a['id']])) ?></td><td role="cell" data-label="<?= it('state') ?>"><?php invoiceStatus($a['status']); ?></td><td role="cell" data-label="<?= it('schedule') ?>"><?php invoiceStatus((int) $a['enabled'] ? 'enabled' : 'disabled'); ?></td></tr><?php endforeach; ?>
...
<section class="card"><h2><?= it('invoices') ?></h2><?php if (!$documents): ?><p><?= it('empty') ?></p><?php else: ?><div class="invoice-table-wrap"><table role="table"><thead role="rowgroup"><tr role="row"><th scope="col" role="columnheader"><?= it('account') ?></th><th scope="col" role="columnheader"><?= it('property') ?></th><th scope="col" role="columnheader"><?= it('number') ?></th><th scope="col" role="columnheader"><?= it('date') ?></th><th scope="col" role="columnheader"><?= it('state') ?></th><th scope="col" role="columnheader"><?= it('company') ?></th><th scope="col" role="columnheader"><?= it('drive') ?></th><th scope="col" role="columnheader"><?= it('toconline') ?></th><th scope="col" role="columnheader"><?= it('download') ?></th></tr></thead><tbody role="rowgroup"><?php foreach ($documents as $d): ?><tr role="row"><td role="cell" data-label="<?= it('account') ?>"><?= ie(InvoiceAccounts::PORTALS[$d['portal']] . ' — ' . $d['account_label']) ?></td><td role="cell" data-label="<?= it('property') ?>"><?= ie($d['property_label'] ?? $d['property_id']) ?></td><td role="cell" data-label="<?= it('number') ?>"><?= ie($d['invoice_number']) ?></td><td role="cell" data-label="<?= it('date') ?>"><?= ie($d['issued_on'] ?? '—') ?><small><?= it($d['period_basis']) ?></small></td><td role="cell" data-label="<?= it('state') ?>"><?php invoiceStatus($d['pipeline_state']); ?></td><td role="cell" data-label="<?= it('company') ?>"><?php invoiceStatus($d['company_state'] ?? 'review'); ?></td><td role="cell" data-label="<?= it('drive') ?>"><?php invoiceStatus('drive_' . ($d['drive_state'] ?? 'pending')); ?><small><?= it('attempts') ?>: <?= (int) ($d['drive_attempts'] ?? 0) ?>/9</small>
<?php if ($d['next_attempt_at']): ?><small><?= it('next_attempt') ?>: <?= ie(invoiceTime($d['next_attempt_at'])) ?></small><?php endif; ?>
<?php if ($d['last_error']): ?><small><?= it($d['last_error']) ?></small><?php endif; ?>
...
</td><td role="cell" data-label="<?= it('toconline') ?>"><?php invoiceStatus($d['toconline_state'] ?? 'pending'); ?></td><td role="cell" data-label="<?= it('download') ?>"><a href="invoice-download.php?document=<?= (int) $d['id'] ?>"><?= ie(strtoupper($d['format'])) ?></a></td></tr><?php endforeach; ?></tbody></table></div><?php endif; ?><?php if ($hasMore): ?><a href="?<?= ie(http_build_query(['account' => $accountId, 'period' => $period, 'before' => end($documents)['id']])) ?>"><?= it('older') ?></a><?php endif; ?><p class="invoice-status-line"><?= it('toconline_setup_pending') ?></p><details class="invoice-options"><summary><?= it('processing_details') ?></summary><p><?= it('airbnb_pipeline') ?></p><p><?= it('toconline_note') ?></p></details></section>
<section class="card"><h2><?= it('latest_run') ?></h2>
<?php if (!$jobs): ?><p><?= it('no_runs') ?></p><?php else: ?><div class="invoice-table-wrap"><table role="table"><thead role="rowgroup"><tr role="row"><th scope="col" role="columnheader"><?= it('created') ?></th><th scope="col" role="columnheader"><?= it('account') ?></th><th scope="col" role="columnheader"><?= it('period') ?></th><th scope="col" role="columnheader"><?= it('state') ?></th><th scope="col" role="columnheader"><?= it('counts') ?></th></tr></thead><tbody role="rowgroup"><?php foreach (array_slice($jobs, 0, 1) as $job): ?><tr role="row"><td role="cell" data-label="<?= it('created') ?>"><?= ie(invoiceTime($job['created_at'])) ?><small><?= it($job['kind']) ?></small></td><td role="cell" data-label="<?= it('account') ?>"><?= ie(InvoiceAccounts::PORTALS[$job['portal']] . ' — ' . $job['account_label']) ?><small><?= ie($properties[$job['account_id']][$job['property_id']] ?? '') ?></small></td><td role="cell" data-label="<?= it('period') ?>"><?= ie($job['period']) ?></td><td role="cell" data-label="<?= it('state') ?>"><?php invoiceStatus($job['state']); ?><?php if ($job['result_code']): ?><small><?= it($job['result_code']) ?></small><?php endif; ?></td><td role="cell" data-label="<?= it('counts') ?>"><?= (int) $job['imported_count'] ?> / <?= (int) $job['duplicate_count'] ?></td></tr><?php endforeach; ?></tbody></table></div>
<?php if (count($jobs) > 1): ?><details class="invoice-options"><summary><?= it('history') ?></summary><div class="invoice-table-wrap"><table role="table"><thead role="rowgroup"><tr role="row"><th scope="col" role="columnheader"><?= it('created') ?></th><th scope="col" role="columnheader"><?= it('account') ?></th><th scope="col" role="columnheader"><?= it('period') ?></th><th scope="col" role="columnheader"><?= it('state') ?></th><th scope="col" role="columnheader"><?= it('counts') ?></th></tr></thead><tbody role="rowgroup"><?php foreach (array_slice($jobs, 1) as $job): ?><tr role="row"><td role="cell" data-label="<?= it('created') ?>"><?= ie(invoiceTime($job['created_at'])) ?><small><?= it($job['kind']) ?></small></td><td role="cell" data-label="<?= it('account') ?>"><?= ie(InvoiceAccounts::PORTALS[$job['portal']] . ' — ' . $job['account_label']) ?><small><?= ie($properties[$job['account_id']][$job['property_id']] ?? '') ?></small></td><td role="cell" data-label="<?= it('period') ?>"><?= ie($job['period']) ?></td><td role="cell" data-label="<?= it('state') ?>"><?php invoiceStatus($job['state']); ?><?php if ($job['result_code']): ?><small><?= it($job['result_code']) ?></small><?php endif; ?></td><td role="cell" data-label="<?= it('counts') ?>"><?= (int) $job['imported_count'] ?> / <?= (int) $job['duplicate_count'] ?></td></tr><?php endforeach; ?></tbody></table></div></details><?php endif; ?>
<?php endif; ?></section>
<?php if ($isGerente): ?>
<section class="card"><h2><?= it('drive') ?></h2>
<p class="invoice-destination">daniel.ciorcas@welcomehostel.pt</p>
<p class="invoice-status-line"><?php invoiceStatus($driveSettings['state'] ?? 'not_configured', match ($driveSettings['state'] ?? 'not_configured') {
    'ready' => 'drive_connection_ready', 'configured' => 'drive_connection_pending', default => 'drive_not_configured',
}); ?></p>
<?php $driveCanConnect = $vault && $vault->has('drive-oauth.enc'); ?>
<?php if (!$driveCanConnect): ?><p><?= it('drive_setup_pending') ?></p><?php endif; ?>
<details class="invoice-options"><summary><?= it('drive_manage') ?></summary>
...
<p><?= it('drive_note') ?></p><p><?= it('drive_setup_note') ?></p></details></section>
<section class="card" id="account-settings"><h2><?= it('settings') ?></h2>
<?php if ($accountId): $a = $repository->get($accountId); ?>
<p class="invoice-status-line"><?php invoiceStatus($a['status']); ?></p>
<details class="invoice-options" open><summary><?= it('manage_account') ?></summary><p><?= $service->browserReady() ? it('browser_ready') : it('preflight_required') ?></p>
...
<details><summary><?= it('sms_device_settings') ?></summary><form method="post" class="form-actions"><?php invoiceHidden('sms_token', $accountId); ?><button class="primary-button"><?= it('sms_token') ?></button></form></details>
...
</details><?php else: ?><p><?= it('select_account') ?></p>
<form method="get" action="invoices.php#account-settings"><input type="hidden" name="period" value="<?= ie($period) ?>"><label class="field"><span><?= it('account') ?></span><select name="account" required><option value=""><?= it('choose_account') ?></option><?php foreach ($accounts as $a): ?><option value="<?= (int) $a['id'] ?>"><?= ie(InvoiceAccounts::PORTALS[$a['portal']] . ' — ' . $a['label']) ?></option><?php endforeach; ?></select></label><button class="primary-button"><?= it('manage_account') ?></button></form><?php endif; ?>
</section>
...
<section class="card"><h2><?= it('failure_alerts') ?></h2><p class="invoice-status-line"><?php invoiceStatus(!empty($notifications['enabled']) ? 'enabled' : 'disabled', !empty($notifications['enabled']) ? 'alerts_enabled' : 'alerts_disabled'); ?></p><p><?= it('alerts_summary') ?></p><details class="invoice-options"><summary><?= it('configure_alerts') ?></summary><form method="post"><?php invoiceHidden('notifications'); ?><div class="form-grid"><label class="field"><span><?= it('manager') ?></span><select name="recipient" required><option value=""><?= it('select_manager') ?></option><?php foreach ($managers as $m): ?><option value="<?= (int) $m['id'] ?>" <?= (int) ($notifications['recipient_user_id'] ?? 0) === (int) $m['id'] ? 'selected' : '' ?>><?= ie($m['display_name']) ?></option><?php endforeach; ?></select></label><label class="field"><span><?= it('template_name') ?></span><input type="text" name="template_name" value="<?= ie($notifications['template_name'] ?? 'invoice_collection_failed_v1') ?>" pattern="[a-z0-9_]+" required></label><label class="check-row"><input type="checkbox" name="enabled" <?= !empty($notifications['enabled']) ? 'checked' : '' ?>><span><?= it('enable_alerts') ?></span></label></div><button class="primary-button"><?= it('save') ?></button></form>
<p><?= it('alerts_note') ?></p></details>
<?php if ($driveAlerts || $alerts): ?><details class="invoice-options"><summary><?= it('alert_history') ?></summary><?php endif; ?>
<?php if ($driveAlerts): ?><ul class="invoice-alerts"><?php foreach ($driveAlerts as $alert): ?><li>Google Drive — <?= ie($alert['account_label']) ?> — <?= ie($alert['period']) ?> — <?= it('alert_' . $alert['state']) ?><br><?= it($alert['error_code']) ?></li><?php endforeach; ?></ul><?php endif; ?>
<?php if ($alerts): ?><ul class="invoice-alerts"><?php foreach ($alerts as $alert): ?><li><?= ie($alert['account_label']) ?> — <?= it('alert_' . $alert['state']) ?></li><?php endforeach; ?></ul><?php endif; ?><?php if ($driveAlerts || $alerts): ?></details><?php endif; ?></section>
<?php endif; ?>
