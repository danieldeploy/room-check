...
$savedAccess=($editing && $vault) ? $repository->credentials($vault,(int)$editing['id']) : [];
?>
...
<?php foreach (['identifier','password'] as $field): ?><label class="field"><span><?= it($field) ?></span><input type="<?= $field==='password'?'password':'text' ?>" name="<?= ie($field) ?>" autocomplete="<?= $field==='password'?'new-password':'username' ?>" maxlength="2048" <?= $field==='identifier'?'value="'.ie((string)($savedAccess['identifier'] ?? '')).'"':'' ?>></label><?php endforeach; ?>
<?php if ($a['portal']==='booking'): ?><div class="invoice-booking-links"><h3><?= it('booking_invoice_links') ?></h3><p><?= it('booking_invoice_links_note') ?></p><?php foreach ($repository->activeProperties($editId) as $propertyId=>$propertyLabel): ?><label class="field"><span><?= ie($propertyLabel) ?></span><input type="url" name="invoice_urls[<?= ie((string)$propertyId) ?>]" value="<?= ie((string)($savedAccess['invoice_urls'][$propertyId] ?? '')) ?>" inputmode="url" maxlength="2048" placeholder="https://admin.booking.com/..."></label><?php endforeach; ?></div><?php endif; ?>
...
<?php if ($a['portal']!=='email'): ?><fieldset data-auth-for="sms" <?= $a['auth_method']!=='sms'?'hidden disabled':'' ?>><h3><?= it('method_sms') ?></h3><p><?= it('sms_note') ?></p><div class="form-grid"><?php foreach (['sms_sender','sms_keyword','sms_sim'] as $field): ?><label class="field"><span><?= it($field) ?></span><input type="text" name="<?= ie($field) ?>" value="<?= ie((string)($savedAccess[$field] ?? '')) ?>" maxlength="190"></label><?php endforeach; ?></div></fieldset><?php endif; ?>
