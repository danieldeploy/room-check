...
try {
    ...
    $pdo->exec("CREATE TABLE invoice_settings (id INTEGER PRIMARY KEY, enabled INTEGER, schedule_day INTEGER, schedule_time TEXT, browser_ready INTEGER, browser_checked_at TEXT, worker_seen_at TEXT, login_verified_at TEXT);
        ...
        CREATE TABLE invoice_jobs (id INTEGER PRIMARY KEY AUTOINCREMENT, kind TEXT, property_id TEXT, period TEXT, state TEXT DEFAULT 'queued', active_key TEXT UNIQUE, schedule_key TEXT UNIQUE, requested_by INTEGER, created_at TEXT, started_at TEXT, finished_at TEXT, result_code TEXT, imported_count INTEGER DEFAULT 0, duplicate_count INTEGER DEFAULT 0);
        CREATE TABLE portal_invoices (id INTEGER PRIMARY KEY AUTOINCREMENT, portal TEXT, property_id TEXT, invoice_number TEXT, issued_on TEXT, period TEXT, sha256 TEXT, size_bytes INTEGER, job_id INTEGER, created_at TEXT, UNIQUE(portal,property_id,invoice_number), UNIQUE(portal,property_id,sha256));");
    ...
    checkInvoice((int) $pdo->query('SELECT count(*) FROM invoice_jobs')->fetchColumn() === 1, 'Disabled means no schedule');
    $service->saveSchedule(true, 5, '04:00');
    $pdo->exec('DELETE FROM invoice_jobs');
    $service->scheduleDue(new DateTimeImmutable('2026-09-05T02:59:00Z'));
    checkInvoice((int) $pdo->query('SELECT count(*) FROM invoice_jobs')->fetchColumn() === 0, 'Lisbon schedule not early');
    $service->scheduleDue(new DateTimeImmutable('2026-09-05T03:00:00Z'));
    checkInvoice((int) $pdo->query('SELECT count(*) FROM invoice_jobs')->fetchColumn() === 2, 'Lisbon summer time schedule');
    $pdo->exec("UPDATE invoice_jobs SET active_key = NULL, state = 'completed'");
    $service->scheduleDue(new DateTimeImmutable('2026-09-15T10:00:00Z'));
    checkInvoice((int) $pdo->query('SELECT count(*) FROM invoice_jobs')->fetchColumn() === 2, 'One scheduled job per property and month');
    $service->scheduleDue(new DateTimeImmutable('2027-01-05T03:59:00Z'));
    checkInvoice((int) $pdo->query('SELECT count(*) FROM invoice_jobs')->fetchColumn() === 2, 'Lisbon winter schedule not early');
    $service->scheduleDue(new DateTimeImmutable('2027-01-05T04:00:00Z'));
    checkInvoice((int) $pdo->query("SELECT count(*) FROM invoice_jobs WHERE period = '2026-12'")->fetchColumn() === 2, 'Year rollover');
...
JS);
    ...
    $pdo->exec('DELETE FROM invoice_jobs; UPDATE invoice_settings SET enabled = 0');
    ...
    $runner->run();
    checkInvoice((int) $pdo->query("SELECT COUNT(*) FROM invoice_jobs WHERE state = 'needs_auth'")->fetchColumn() === 2, 'Auth failure stops remaining queue');
    ...
    $runner->run();
    checkInvoice($pdo->query("SELECT state FROM invoice_jobs WHERE id = $retry")->fetchColumn() === 'completed', 'Empty confirmed period completes');
    checkInvoice(!empty($service->settings()['login_verified_at']), 'Successful portal read validates access');
    $pdo->exec("UPDATE invoice_jobs SET state = 'running' WHERE id = $retry");
    $runner->run();
    checkInvoice($pdo->query("SELECT result_code FROM invoice_jobs WHERE id = $retry")->fetchColumn() === 'interrupted', 'Interrupted jobs do not remain running');
    file_put_contents($tmp . '/fixture-code', 'throw');
    $retry = $service->enqueue('collect', '1140306', '2026-08', 1);
    $runner->run();
    checkInvoice($pdo->query("SELECT result_code FROM invoice_jobs WHERE id = $retry")->fetchColumn() === 'worker_failed', 'Child errors are sanitized');
