...
// letters are treated as likely typing corruption before PT sentence context can
...
assertHardening(str_contains($guardSource, 'LexicalNearMatchClassifier'), 'near-known typo evidence is checked before sentence fallback');
assertHardening(!str_contains($guardSource, '/^[\\p{Lu}]{2,}$/u'), 'all-uppercase words are no longer automatically accepted as technical identifiers');
...
assertHardening(str_contains($lexicalSource, 'likelyMisspelling') && str_contains($lexicalSource, 'unicodeEditDistance'), 'local lexical checker performs bounded UTF-8 near-match checks');
