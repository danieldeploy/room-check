...
try {
    ...
    if ($PrepareOnly) {
        & $node (Join-Path $app 'diagnose-browser.mjs')
        if ($LASTEXITCODE -ne 0) { throw 'Chrome preparation test failed.' }
        Write-Host 'Files and Chrome prepared. No pairing key, scheduled task or portal collection has been activated.'
        Write-Host 'Next use Pair-Agent.ps1, then Run-Agent.ps1 -TestOnly before scheduling or activation.'
        exit 0
    }
    ...
    & powershell.exe -NoProfile -ExecutionPolicy RemoteSigned -File (Join-Path $app 'windows\Run-Agent.ps1') -TestOnly
